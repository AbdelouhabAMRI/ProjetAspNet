...
if ($foundDbContextType) {
	...
	$isAssignableToParamTypes[0] = [ICSharpCode.PackageManagement.EnvDTE.CodeType]
	...
	if (!$isDbContext) {
		...
		$hasExistingPropertyForModel = $existingMembers | ?{ ($_.Name -eq $propertyName) -and ($_.Kind -eq [ICSharpCode.PackageManagement.EnvDTE.vsCMElement]::vsCMElementProperty) }
